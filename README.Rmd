...
**A lot has changed with `tvthemes 1.0.0` so please check the [tvthemes 1.0.0](#tvthemes-100-major-changes) section below rather than the blogpost.**
...
```{r, fig.width=17, fig.height = 10, message=FALSE}
airquality %>% 
  mutate(Month = as.factor(Month)) %>% 
  ggplot(aes(x = Day, y = Temp, group = Month, color = Month)) +
  geom_line(size = 1.5) +
